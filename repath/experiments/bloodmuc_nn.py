...
pixel_feature_detector = PixelFeatureDetector(features_list=features_list, sigma_min = 1, sigma_max = 16)
...
def train_classifier() -> None:
    ...
    # train classifier
    clf = MLPClassifier(hidden_layer_sizes=[50, 50, 50], max_iter=500)
    clf.fit(feats_sample, label_sample.ravel())
    print("save: ", experiment_root, check_is_fitted(clf))
    experiment_root.mkdir(parents=True, exist_ok=True)
    dump(clf, experiment_root / 'nn_mlp.joblib') 
...
def predict_images() -> None:
    ...
    # load classifier
    clf = load(experiment_root / 'nn_mlp.joblib')
