...
import repath.data.datasets.cervical_set2 as cervical
...
Global stuff
"""
experiment_name = "cervical_set2_exp1"
...
class PatchClassifier(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        total=len(y)   
        accu = correct / total
        self.log("train_accuracy", accu, on_step=True, on_epoch=True)
...
def preprocess_indexes() -> None:
    ...
    train_data = cervical.set2()
    patch_finder = GridPatchFinder(labels_level=5, patch_level=0, patch_size=128, stride=128, border=128)
...
def preprocess_samples() -> None:
    ...
    train_samples = balanced_sample([train], 600000, floor_samples=500000)
    valid_samples = balanced_sample([valid], 300000, floor_samples=250000)
...
def inference_on_valid_pre() -> None:
    ...
    output_dir = experiment_root / "patch256_results" / "valid"

    results_dir_name = "results"
    heatmap_dir_name = "heatmaps"

    valid_patches = SlidesIndex.load(cervical.set2(), experiment_root / "valid_index")
...
def inference_on_train_pre() -> None:
    ...
    output_dir = experiment_root / "patch256_results" / "train"

    results_dir_name = "results"
    heatmap_dir_name = "heatmaps"

    train_patches = SlidesIndex.load(cervical.set2(), experiment_root / "train_index")
...
def calculate_patch_level_results() -> None:
    ...
    dirin = experiment_root / 'patch256_results' / 'valid'
    dirout = experiment_root / 'patch256_results' / "patch_summaries" / 'valid'

    val_results = SlidesIndexResults.load(cervical.set2(), dirin, "results", "heatmaps")

    title = experiment_name + ' experiment ' + 'patch256' + ' model Cervical ' + 'valid' + ' dataset'
