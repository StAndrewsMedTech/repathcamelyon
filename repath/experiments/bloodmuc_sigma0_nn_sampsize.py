...
def train_classifier() -> None:
    ...
    output_array = np.hstack((label_sample, feats_sample))
    experiment_root.mkdir(parents=True, exist_ok=True)
    np.savetxt(experiment_root / 'samples.csv', output_array, delimiter=",")
    # train classifier
    clf = MLPClassifier(hidden_layer_sizes=[50, 50, 50], max_iter=500, verbose=True)
