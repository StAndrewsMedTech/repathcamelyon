...
def inference_on_valid_pre() -> None:
    ...
    output_dir = experiment_root / "pre_hnm_results" / "valid"
...
def calculate_patch_level_results() -> None:
    ...
    dirin = experiment_root / 'pre_hnm_results' / 'valid'
    dirout = experiment_root / 'pre_hnm_results' / "patch_summaries" / 'valid'

    val_results = SlidesIndexResults.load(cervical.debug(), dirin, "results", "heatmaps")

    title = experiment_name + ' experiment ' + 'pre_hnm' + ' model Cervical ' + 'valid' + ' dataset'
